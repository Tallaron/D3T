...
<div class="col-sm-4">
    <select size="1" id="skill-a-{$key}" data-rune="rune-a-{$key}" name="skill-a[{$key}][skill]" class="form-control custom-select-untouched">
        ...
        {foreach from=$lists->getActiveSkills() item=$skill}
            <option value="{$skill->getId()}" class="custom-select-default"{if !$build->getActiveSkills()->isEmpty() && $skill->getId() == $build->getActiveSkills()->getSkillAt($key)->getSkillId()} selected{/if}>{$skill->getName()}</option>
        {/foreach}
    </select>
</div>
