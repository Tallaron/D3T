...
<div class="col-sm-4">
    <select size="1" id="skill-p-{$key}" name="skill-p[{$key}][skill]" class="form-control custom-select-untouched">
        ...
        {foreach from=$lists->getPassiveSkills() item=$skill}
            <option value="{$skill->getId()}" class="custom-select-default"{if !$passiveSkills->isEmpty() && $skill->getId() == $passiveSkills->getSkillAt($key)->getSkillId()} selected{/if}>{$skill->getName()}</option>
        {/foreach}
    </select>
</div>
